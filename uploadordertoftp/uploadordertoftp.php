...
class UploadOrderToFTP extends Module {
	...
	$this->salt = 'This is the key with which the data is encrypted.';
    }
	...
	$output = $this->context->smarty->fetch(dirname(__FILE__).'/views/templates/hook/order.tpl');

	$this->file_name = str_pad($order->id, 6, '0', STR_PAD_LEFT);
	...
	$remote_file = $ftp_path.$this->file_name.'.txt';
	...
	{
	    $pvp_ftp_server = (string)Tools::getValue('PVP_FTP_SERVER');
	    $pvp_ftp_path = (string)Tools::getValue('PVP_FTP_PATH');
	    $pvp_ftp_username = (string)Tools::getValue('PVP_FTP_USERNAME');
	    $pvp_ftp_password = (string)Tools::getValue('PVP_FTP_PASSWORD');
	    $pvp_ftp_passive = (int)Tools::getValue('PVP_FTP_PASSIVE');
	...
	return trim(base64_encode(mcrypt_encrypt(MCRYPT_RIJNDAEL_256, $this->salt, $text, MCRYPT_MODE_ECB, $iv)));
    }

    private function decrypt($text)
    {
	$iv = mcrypt_create_iv(mcrypt_get_iv_size(MCRYPT_RIJNDAEL_256, MCRYPT_MODE_ECB), MCRYPT_RAND);
	return trim(mcrypt_decrypt(MCRYPT_RIJNDAEL_256, $this->salt, base64_decode($text), MCRYPT_MODE_ECB, $iv));
    }
